...
#include "helper_classes/constants.h"
...
SplitLineSegmentPairs RotationalSymmetry::splitLineSegmentsInPairs(const std::vector<LineSegment>& lineSegments, const VoxelMesh& voxelMesh, const double lengthTolerance, const double angleTolerance) {
    ...
    auto splitLineSegments = SymmetryFunctions::splitLineSegmentVectorIntoParts(lineSegments, lengthTolerance);
        ...
        for (unsigned long long i = 0; i < part.size() - 1; i++) {
            for (unsigned long long j = i + 1; j < part.size(); j++) {
                ...
                if (lineSegmentPairs[level].empty()){
                    ...
                    for (unsigned int layer = 0; layer < static_cast<unsigned int>(voxelMesh.voxelZ); layer++) {
                        lineSegmentPairs[level][0].push_back(std::vector<LineSegmentPair>());
                    }

                    const unsigned int layer = static_cast<unsigned int>(part[i].p1.z / voxelMesh.voxelSideSize);
                ...
                if (foundIndex == -1) {
                    ...
                    for (uint layer = 0; layer < voxelMesh.voxelZ; layer++) {
        ...
        for (unsigned int distanceGroup = 0; distanceGroup < pairs[level].size(); distanceGroup++) {
            for (unsigned int layer = 0; layer < pairs[level][distanceGroup].size(); layer++) {
                for (const LineSegmentPair& pair : pairs[level][distanceGroup][layer]) {
                    ...
                    const double distance = PointFunctions::distance(pair.ls1.getCenterPoint(), pair.ls2.getCenterPoint());
    ...
    for (const LineSegmentSubgroup& subgroup : subgroups) {
        ...
        for (unsigned int i = 0; i < subgroup.lineSegments.size() - 1; i++) {
            for (unsigned int j = i + 1; j < subgroup.lineSegments.size(); j++) {
                ...
                const Point midPoint1 = ls1.getCenterPoint();
                const Point midPoint2 = ls2.getCenterPoint();

                const LineSegment connection(midPoint1, midPoint2);

                // Calculation of the bisection,for [i][j].
                LineSegment bisection = connection.calculateBisection();
        ...
        for (const std::vector<int>& combination : combinations) {
            ...
            Point center;
            LineSegmentFunctions::calculateIntersetion(center, bisection1, bisection2);
            const double radius = PointFunctions::distance(center, ls1.getCenterPoint());
    ...
    for (unsigned long long i = 0; i < rotationalSymmetries.size(); i++) {
        ...
        for (const LineSegment& ls : rotationalSymmetries[i].lineSegments) {
            // First line segment point.
            {
                ...
                const Point p = ls.p1;
                const int x = static_cast<int>(floor((p.x - voxelMesh.minX) / voxelMesh.voxelSideSize));
                const int y = static_cast<int>(floor((p.y - voxelMesh.minY) / voxelMesh.voxelSideSize));
                const int z = static_cast<int>(floor((p.z - voxelMesh.minZ) / voxelMesh.voxelSideSize));
        ...
        const unsigned int x = static_cast<unsigned int>(floor((p.x - voxelMesh.minX) / voxelMesh.voxelSideSize));
        const unsigned int y = static_cast<unsigned int>(floor((p.y - voxelMesh.minY) / voxelMesh.voxelSideSize));

        for (unsigned int z = 0; z < static_cast<unsigned int>(voxelMesh.voxelZ); z++) {
...
std::vector<PositionFromPlane> RotationalSymmetry::calculatePositionsFromAxis(const std::vector<Point>& points, const VoxelMesh& voxelMesh, const bool validAxis) {
    ...
    for (unsigned long long i = 0; i < numberOfPoints; i++) {
        ...
        const int voxelX = static_cast<int>(floor((point.x - voxelMesh.minX) / static_cast<float>(voxelMesh.voxelSideSize)));
        const int voxelY = static_cast<int>(floor((point.y - voxelMesh.minY) / static_cast<float>(voxelMesh.voxelSideSize)));
        const int voxelZ = static_cast<int>(floor((point.z - voxelMesh.minZ) / static_cast<float>(voxelMesh.voxelSideSize)));
...
{
    std::vector<LineSegment> lineSegments = SymmetryFunctions::calculateLineSegmentsBetweenPoints(points, voxelVector, voxelMesh, lengthTolerance, minimumLineSegmentDistance);  // Calculation of line segments betweeen all pairs of points.
    SplitLineSegmentPairs splitLineSegments = splitLineSegmentsInPairs(lineSegments, voxelMesh, lengthTolerance, angleTolerance);  // Splitting the line segments.
    std::vector<Circumcircle> circumcircles = buildLineSegmentMidpointsCircumcircles(splitLineSegments);                           // Building circumcircles.
    std::vector<RotationalSymmetry> symmetries = findSimpleSymmetries(circumcircles, angleTolerance);                              // Finding simple symmetries.
    std::vector<RotationalSymmetry> mergedSymmetries = mergeSymmetries(symmetries);                                                // Merging symmetries.
    getVoxelsInSymmetries(mergedSymmetries, voxelVector, voxelMesh);                                                               // Adding voxels to symmetries.
...
void Circumcircle::addLineSegment(const LineSegment& ls) {
    const Vector3d v = (ls.getCenterPoint() - center).normalize();
    ...
    layer(layer)
{
    ...
    if (!pairs.empty()) {
        distanceBetweenPairCenterPoints = PointFunctions::distance(pairs[0].ls1.getCenterPoint(), pairs[0].ls2.getCenterPoint());
    }
}
