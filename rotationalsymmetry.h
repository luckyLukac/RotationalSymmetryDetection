...
namespace Symmetry {
    ...
    private:
        ...
        // CLASS METHODS
        static SplitLineSegmentPairs splitLineSegmentsInPairs(const std::vector<LineSegment>& lineSegments, const VoxelMesh& voxelMesh, const double lengthTolerance, const double angleTolerance);  // Finding simple symmetries.
        static void buildCombinationsVectorRecursively(std::vector<std::vector<int> >& combinations, std::vector<int>& tempVector, int N, int left, int K);  // Getting one recursive vector combination.
        static std::vector<std::vector<int>> allVectorCombinations(const int N, const int K);  // Getting all vector combinations with size K from a vector with the size of N.
        static std::vector<LineSegmentPairGroup> splitPairsIntoGroups(const SplitLineSegmentPairs& pairs);  // Splitting line segment pairs by length between pair center points.
        static std::vector<LineSegmentSubgroup> splitLineSegmentGroupsIntoSubgroups(const std::vector<LineSegmentPairGroup>& groups);  // Splitting line segment groups into subgroups.
        static std::vector<Circumcircle> buildLineSegmentMidpointsCircumcircles(const SplitLineSegmentPairs& pairs);  // Building line segment midpoints circumcircles.
        static std::vector<RotationalSymmetry> findSimpleSymmetries(const std::vector<Circumcircle>& circumcircles, const double angleTolerance);  // Finding simple rotational symmetries from circumcircles.
        static std::vector<RotationalSymmetry> mergeSymmetries(std::vector<RotationalSymmetry>& symmetries);  // Merging symmetries by layers, where symmetries with the same rotation axis and rotation level are merged.
        static void getVoxelsInSymmetries(std::vector<RotationalSymmetry>& rotationalSymmetries, const VoxelVector& voxelVector, const VoxelMesh& voxelMesh);  // Getting voxels for each rotational symmetry.
    public:
        // OBJECT METHODS
        ...
        std::vector<PositionFromPlane> calculatePositionsFromAxis(const std::vector<Point>& points, const VoxelMesh& voxelMesh, bool validAxis);  // Calculating the point positions according to the axis.
        ...
        // CLASS METHODS
        static std::vector<RotationalSymmetry> calculateRotationalSymmetries(const std::vector<Point>& points, const VoxelVector& voxelVector, const VoxelMesh& voxelMesh, const double lengthTolerance, const double angleTolerance, const double minimumLineSegmentDistance = 1);  // Calculating rotational symmetries.
    };
